...
public class EmployeeProfile implements Initializable, Controller {
    @FXML
    TextField txtFname,txtLname,txtEmail,txtNatId,txtPassword;
    @FXML
    Label lblTitle,lblPasswordError,lblEmailError;
    String update="Update employee set firstName=?,lastName=?,email=?,nationalId=? where id =?";
    String updatePassword="Update employee set firstName=?,lastName=?,email=?,nationalId=?,password=? where id =?";
    String getAgent="Select * from employee where id=?";
    ...
    public void goBack() throws Exception {
        lblTitle.getScene().setRoot(new Dashboard().getContent());
    }
    @Override
    ...
    public Parent getContent() throws Exception {
        return FXMLLoader.load(AddAgent.class.getResource("/ml/allansifuna/fxmls/EmployeeProfile.fxml"));
    }
}
